...
<header>
    ...
    <div class="div-title-admin">
        <div class="admin-container">
            <div class="add-btn-divs">
                <div class="div-title-admin">
                    <h1 class="title-admin">Заказы</h1>
                </div>
                <div class="btn_add">
                    <a href="./create_Type-product.php">Добавить запись</a>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="admin-container">
            <div class="table_row title">
                <div class="table_sell id">
                    <p class="admin_table_cell_first-order">ID</p>
                </div>
                <div class="table_sell">
                    <p class="admin_table_cell-text-order">Заказчик</p>
                </div>
                <div class="table_sell">
                    <p class="admin_table_cell-text-order">Продукт</p>
                </div><div class="table_sell">
                    <p class="admin_table_cell-text-order">Мастер</p>
                </div><div class="table_sell">
                    <p class="admin_table_cell-text-order">Дата оформления</p>
                </div><div class="table_sell">
                    <p class="admin_table_cell-text-order">Цена</p>
                </div><div class="table_sell">
                    <p class="admin_table_cell-text-order">Статус заказа</p>
                </div><div class="table_sell">
                    <p class="admin_table_cell-text-order">Дата окончания</p>
                </div>
            </div>
        </div>
    </div>
    ...
    if (isset($_GET['id_orders'])) {
        $Orders = $_GET['id_orders'];
        $deleteOrders = "DELETE FROM `Orsers` WHERE id_orders='$Orders'";
        addslashes($deleteOrders);
        $resDeleteTrack = mysqli_query($connect, $deleteOrders) or die(mysqli_error($connect)); 
    }

    $infoOrders = "SELECT * FROM Orders, Clients, Products, Masters 
                WHERE Orders.id_client = Clients.id_client 
                AND Orders.id_product = Products.id_product 
                AND Orders.id_master = Masters.id_master";
                    // ,`ID_product` = Products.id_product , `ID_master` = Masters.id_master
    addslashes($infoOrders);
    $resinfoOrders = mysqli_query($connect, $infoOrders) or die(mysqli_error($connect));
    while ($infoOrders = mysqli_fetch_object($resinfoOrders)) {
        echo "
    <div class=\"body-table\">
        <div class=\"admin-container\">
            <div class=\"admin_table_row\">
                <div class=\"admin_table_cell_first admin_projects_table_id\">
                    <p>$infoOrders->id_order</p>
                </div>
                <div class=\"admin_table_cell admin_projects_table_name\">
                    <p class=\"admin_table_cell-text\">$infoOrders->id_client</p>
                </div>
                <div class=\"admin_table_cell admin_projects_table_name\">
                    <p class=\"admin_table_cell-text\">$infoOrders->id_product </p>
                </div>
                <div class=\"admin_table_cell admin_projects_table_name\">
                    <p class=\"admin_table_cell-text\">$infoOrders->id_master</p>
                </div>
                <div class=\"admin_table_cell admin_projects_table_name\">
                    <p class=\"admin_table_cell-text\">$infoOrders->date_order</p>
                </div>
                <div class=\"admin_table_cell admin_projects_table_name\">
                    <p class=\"admin_table_cell-text\">$infoOrders->price</p>
                </div>
                <div class=\"admin_table_cell admin_projects_table_name\">
                    <p class=\"admin_table_cell-text\">$infoOrders->status</p>
                </div>
                <div class=\"admin_table_cell admin_projects_table_name\">
                    <p class=\"admin_table_cell-text\">$infoOrders->date_end</p>
                </div>
                
                <div class=\"admin_projects_table_act\">
                    <a href=\"edit_Users-jewel.php?idA=$infoOrders->id_order\">Изменить</a>
                <a href=\"?id_order=$infoOrders->id_order\">Удалить</a>
            </div>
        </div>
    </div>
";
