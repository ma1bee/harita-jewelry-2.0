...
<header>
    ...
    <div class="div-title-admin">
        <div class="admin-container">
            <div class="add-btn-divs">
                <div class="div-title-admin">
                    <h1 class="title-admin">Тип продукции</h1>
                </div>
                <div class="btn_add">
        ...
        <div class="admin-container">
            <div class="table_row title">
                ...
                <div class="table_sell">
                    <p class="admin_table_cell-text">Продукт</p>
                </div>
            </div>
        </div>
    </div>
    ...
    if (isset($_GET['id_type_product'])) {
        $TypeProduct = $_GET['id_type_product'];
        $deleteTypeProduct = "DELETE FROM `Type_products` WHERE id_type_product='$TypeProduct'";
        addslashes($deleteTypeProduct);
        $resDeleteTrack = mysqli_query($connect, $deleteTypeProduct) or die(mysqli_error($connect)); 
    }

    $infoTypeProduct = "SELECT *
                    FROM Type_products";
    addslashes($infoTypeProduct);
    $resinfoTypeProduct = mysqli_query($connect, $infoTypeProduct) or die(mysqli_error($connect));
    while ($infoTypeProduct = mysqli_fetch_object($resinfoTypeProduct)) {
        echo "
    <div class=\"body-table\">
        <div class=\"admin-container\">
            <div class=\"admin_table_row\">
                <div class=\"admin_table_cell_first admin_projects_table_id\">
                    <p>$infoTypeProduct->id_type_product</p>
                </div>
                <div class=\"admin_table_cell admin_projects_table_name\">
                    <p class=\"admin_table_cell-text\">$infoTypeProduct->name_type_product</p>
                </div>
                
                <div class=\"admin_projects_table_act\">
                    <a href=\"edit_Users-jewel.php?idA=$infoTypeProduct->id_type_product\">Изменить</a>
                <a href=\"?id_type_product=$infoTypeProduct->id_type_product\">Удалить</a>
            </div>
        </div>
    </div>
";
